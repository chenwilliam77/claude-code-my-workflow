...
<details>
<summary><strong>35 agents, 54 skills, 35 rules, 7 hooks</strong> (click to expand)</summary>
...
| `data-analysis-coder` | Computational partner to the contractor — implements a stage's analysis code in whichever language fits, gated on a stated golden-value cross-check |
| `data-analysis-project-contractor` | Orchestrates a multi-stage data-analysis project end to end — intent refinement, per-stage dispatch, code review, verification, mandatory golden-value sanity checks |
| `haiku-proofreader` | Haiku-tier proofreading agent — grammar, typos, overflow, consistency, for mechanical pattern checks |
| `haiku-verifier` | Haiku-tier end-to-end verification agent — compile/render/deploy checks for purely mechanical exit-code/grep checks |
| `sonnet-beamer-translator` | Sonnet-pinned Beamer-to-Quarto translation specialist |
| `sonnet-claim-verifier` | Sonnet-pinned Chain-of-Verification fact-checker in a forked context |
| `sonnet-domain-referee` | Sonnet-pinned substantive manuscript referee for `--peer` mode |
| `sonnet-domain-reviewer` | Sonnet-pinned substantive domain review template for lecture slides |
| `sonnet-editor` | Sonnet-pinned journal editor for `/review-paper --peer` (desk review + referee selection + synthesis) |
| `sonnet-methods-referee` | Sonnet-pinned paper-type-aware methodology referee |
| `sonnet-pedagogy-reviewer` | Sonnet-pinned holistic pedagogical reviewer for academic slides |
| `sonnet-quarto-critic` | Sonnet-pinned adversarial QA agent comparing Quarto HTML against Beamer PDF |
| `sonnet-quarto-fixer` | Sonnet-pinned implementer of quarto-critic fixes |
| `sonnet-r-reviewer` | Sonnet-pinned R code reviewer for academic scripts |
| `sonnet-slide-auditor` | Sonnet-pinned visual layout auditor for RevealJS/Beamer slides |
| `sonnet-tikz-reviewer` | Sonnet-pinned harsh TikZ diagram critic |
| `sonnet-verifier` | Sonnet-pinned end-to-end verification agent |
