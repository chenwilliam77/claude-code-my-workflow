...
- **INV-13: Tables are generated, never hand-typed.** Every table cell that appears in a `.tex` or `.html` file must originate from a script's programmatic output (`modelsummary`/`stargazer` export, or values read from a csv/rds/txt and formatted in code) — either `\input{}`-ed directly or produced by a script and copied verbatim from that output. The LLM must never compute or transcribe a table value by hand into source. If a table isn't yet script-generated, generate it (or fix the script) before writing it into a `.tex`/`.html` file — do not hand-type a placeholder or "reasonable-looking" number as a stopgap.
