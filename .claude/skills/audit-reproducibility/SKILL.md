...
argument-hint: "[manuscript path] [outputs-dir] (outputs-dir defaults to programs/R/_outputs/)"
...
- `$1` — path to the outputs directory. Defaults to `programs/R/_outputs/`. Recognised alternatives: `programs/stata/_outputs/` (Stata pipelines built by [`/stata-replication`](../stata-replication/SKILL.md)), `_targets/objects/` (R `targets` workflows), any directory the user-specified outputs live in.
...
2. Verify the outputs directory exists and is non-empty. If empty or stale (older than the manuscript), prompt the user to re-run their pipeline (e.g., `Rscript programs/R/00_run_all.R`) before auditing.
...
```
{
  source: "programs/R/_outputs/results.rds",
